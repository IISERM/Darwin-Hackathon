...
Human population is an `nXn` grid with each cell representing `h` humans(hosts) and `m` Anopheles Stephensi mosquitoes(vector). Assume that the number of infected of infected humans in each cell is drawn from a uniform distribution between `0` and `h` and the number of mosquitoes in each cell is drawn from a uniform distribution between `0` and `m`. The host and the vector grids are updated in the following manner:
