...
Wonderful! Now we have a terrain generator. Now to simulate organisms on it. Each organism is capable of reproduction in iteration after its birth. Reproduction is defined as follow. Each organism will look at the neighbouring empty cell out of the four neighbouring cells, select one and with a probability p, reproduce. If two or more organisms select the same cell, the organisms fight and all but one of them dies while the one that lives doesn’t reproduce. An organism may die in an iteration with probability d before reproduction. The type of terrain affects p, d and the outcome of fight during reproduction. If the terrains of the reproducing organisms are different, if at least one reproducing organism has the same terrain type as the unoccupied cell, the one with a different terrain type as the unoccupied cell loses the fight and dies and the remaining organisms on same terrain as unoccupied cell fight till one survives (Choose 1 randomly). In all other cases, both organisms have a 50-50 chance.
